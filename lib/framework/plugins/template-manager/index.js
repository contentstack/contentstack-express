                                    ...
                                    for (var i = 0, _i = schema.length; i < _i; i++) {
                                        ...
                                        if (schema[i].multiple) {
                                            ...
                                            switch (type) {
                                                ...
                                                case "group":
                                                    if (blocks && typeof blocks === 'string')
                                                        generator(tabs + "\t\t\t", "_" + _temp, field.schema, blocks);
                                                    else
                                                        generator(tabs + "\t\t\t", "_" + _temp, field.schema);
                                                    break;
                                                case "blocks":
                                                    for (var j = 0; j < field.blocks.length; j++) {
                                                        content += tabs + "\t\t{% if _" + _temp + "." + field.blocks[j].title.toLowerCase() + "." + field.blocks[j].schema[0].uid + " %}";
                                                        content += tabs + "\t\t\t<div class='block-field'><div class='key'>" + field.blocks[j].title + "</div>";
                                                        generator(tabs + "\t\t\t\t", "_" + _temp, field.blocks[j].schema, field.blocks[j].title.toLowerCase());
                                                        content += tabs + "\t\t\t</div>";
                                                        content += tabs + "\t\t{% endif %}";
                                                    }
                                                    break;
                                            ...
                                            switch (type) {
                                                ...
                                                case "blocks":
                                                    content += tabs + "<div class='field'>";
                                                    content += tabs + "<div class='key'>" + field.display_name + "</div>";
                                                    content += tabs + "<div class='blocks-field'>";
                                                    if (blocks && typeof blocks === 'string')
                                                        generator(tabs + "\t", prefix + "." + _tmp_field, field.schema);
                                                    else
                                                        generator(tabs + "\t", prefix + "." + _tmp_field, field.schema);
