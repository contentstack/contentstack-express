...
app.fuse = function () {
  ...
  if (_static) {
    var _path = (path.isAbsolute(_static.path)) ? _static.path : path.join(config.get('path.theme'), (_static.path ||
      './public'));
...
app.listen = function () {
  ...
  var stack = this._router.stack,
    ...
    _error = [],
    _staticRoutes = [],
    publicStatic = _.findIndex(stack, {
      name: 'serveStatic'
    });
  var i = 0,
    flag = false;
  while (i < stack.length) {
    if (stack[i] && typeof stack[i] === 'object' && stack[i]['name'] && stack[i]['name'] === 'serveStatic' && i !==
      publicStatic) {
   ...
   * */
  _middlewares = [(_.findLastIndex(stack, {
    name: 'serveStatic'
  }) + 1), 0].concat(_staticRoutes);
...
/**
 * Method that sends respnse to the client
 * @return {Function} : Responsds to the client OR calls the next middleware
 */
function respond () {
