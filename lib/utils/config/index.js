...
function Config() {
  try {
    ...
    const server = process.env.SERVER || process.env.server || 'default';
    ...
    const _all = fs.existsSync(path.join(_path, 'all.js')) ? require(path.join(_path, 'all')) : {};
    const _env = fs.existsSync(path.join(_path, env + '.js')) ? require(path.join(_path, env)) : {};

    // merge default configuration with specified environment
    const _config = _.merge(_default, _.merge(_all, _env));
    // set other configurations
    const _theme = path.join(_dirname, 'themes', _config.theme);
      ...
      if (typeof _config.view.custom_templates === 'string' && fs.existsSync(path.join(_dirname, _config.view.custom_templates))) {
        ...
        _config.view.custom_templates.forEach(function(template_path) {
          if (fs.existsSync(_dirname, template_path)) {
            ...
            console.error(`${template_path} provided in config failed to resolve.`);
          }
        });
      } else {
        console.error(`${_config.view.custom_templates} provided in config failed to resolve.`);
      }
    }
    ...
    console.error(`Error in loading config\n${e.message || error}`);
    process.exit(1);
  }
}
...
function languages(config) {
  const _languages = config.languages;
  const storage = config.storage;
  const contentPath = config.path.storage;
  const keys = [];
  const opts = {
    sensitive: true,
    strict: true,
  ...
  const idx = _.findIndex(_languages, {
    relative_url_prefix: '/'
  });
  if (~idx) {
...
function storage(_assets) {
  ...
  if (_assets.pattern) {
    const split = _assets.pattern.split('/:');
    _.merge(_assets, {
