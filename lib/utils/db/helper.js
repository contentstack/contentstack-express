...
var skipFormIds = (process.env.SYNC !== 'false') ? ['_routes', '_content_types',
  '_assets'
] : [];
...
exports.generateCTNotFound = function(locale, contentTypeUID) {
  ...
  if (!jsonPath || !fs.existsSync(jsonPath)) {
    error = new Error(`The content type '${contentTypeUID}' did not exist`);
    error.code = 422;
    throw error;
  }
};
...
exports.filterQuery = function(_query, build) {
  ...
  var __keys = ['locale', '_remove', 'include_references', 'include_count',
    '_include_previous', '_include_next', '_referenceDepth'
  ];
 ...
 * @param  {Object} source      : Object being copied
...
exports.findReferences = function(contentType) {
  if (contentType && contentType.schema && contentType.schema.length) {
    var _data = {},
      _keys = ['title', 'uid', 'schema', 'options', 'singleton', 'references',
    ...
    var _findReferences = function(_schema, parent) {
      for (var i = 0, total = _schema.length; i < total; i++) {
        var parentKey;
        if (_schema[i] && _schema[i]['data_type'] && _schema[i]['data_type'] ===
          'reference') {
          var field = ((parent) ? parent + ':' + _schema[i]['uid'] :
            _schema[i]['uid']);
          _data[field] = _schema[i]['reference_to'];
        } else if (_schema[i] && _schema[i]['data_type'] && _schema[i][
            'data_type'
          ] === 'group' && _schema[i]['schema']) {
          _findReferences(_schema[i]['schema'], ((parent) ? parent + ':' +
            _schema[i]['uid'] : _schema[i]['uid']));
        }
      }
    };
...
exports.queryBuilder = function(query, language, content_type_id, callback) {
  ...
  if (query && Object.keys(query).length && content_type_id && skipFormIds.indexOf(
      content_type_id) === -1) {
    var Inmemory = require('./inmemory/index'),
      schema = Inmemory.get(language, '_content_types', {
          ...
          return function(_callback) {
            ...
            for (var refField in references) {
              ...
              if (filterField.indexOf(newRefField) === 0) {
                ...
                refQuery = refQuery || {};
                var newFilterField = filterField.replace(newRefField,
                  '_data'); // remove this entry, replacement if system going to attach the '_data.'
                refQuery[newFilterField] = query[filterField];
                delete query[filterField];
              }
            }
            if (refQuery && Object.keys(refQuery).length) {
              _calls[_filterField] = (function(refQuery,
                content_type_id) {
                return function(_cb) {
                  var RefData = Inmemory.get(language,
                      content_type_id, refQuery),
                    RefQuery = {
                      '$in': []
                    };
                  if (RefData && RefData.length) RefQuery = {
                    $in: _.map(RefData, 'uid')
                  };
                  _cb(null, RefQuery);
                }
              }(refQuery, refForm));
              ...
              var __calls = [];
              for (var i = 0, total = query[filterField].length; i <
                total; i++) {
                __calls.push(function(filterQuery) {
                  return function(__cb) {
                    utility.queryBuilder(filterQuery, language,
                      content_type_id, __cb);
                  }
                }(query[filterField][i]));
              }
...
exports.getContentPath = function(langCode) {
  ...
  if (~idx) {
    ...
    console.error(`{langCode} language does not exist`);
  }
};
  ...
  try {
    var result, options, _query,
      ...
      callback = function(operation, excludeFlag) {
        return function(err, data) {
          try {
            self._locale = self._operation = self._uid = self.single = self
              .json = null;
            if (err) throw err;
            if ((operation === 'fetch' || operation === 'find') && !
              excludeFlag) removeUnpublishedAssets(data);
    ...
    switch (self._operation) {
      ...
      case 'insert':
        ...
        if (self._uid || (_query._data && _query._data.uid)) _query._uid =
          self._uid || _query._data.uid;
      ...
      case 'find':
        ...
        _query = _.clone(self._query, true);
        utility.queryBuilder(_query, self._locale, self.content_type_id,
          function(error, resultQuery) {
            ...
            if (skipFormIds.indexOf(self.content_type_id) === -1) {
              ...
              var results = Inmemory.get(self._locale, self.content_type_id,
                _query);
              ...
              callback(self._operation, excludeUnpublishDeletionFlag)(null,
                results);
            }
          });
        break;
...
function removeUnpublishedAssets(data) {
  function _removeUnpublishedAssets(objekt, parent, pos) {
    if (_.isPlainObject(objekt) && _.has(objekt, 'filename') && _.has(objekt,
        'uid') && !_.has(objekt, '_internal_url')) {
