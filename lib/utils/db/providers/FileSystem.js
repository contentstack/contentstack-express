...
fileStorage.prototype.findOne = function (query, callback) {
    ...
    try {
        if (query && typeof query === 'object') {
            ...
            if (jsonPath) {
                fs.stat(jsonPath, function (error, stat) {
                    if (error) {
                        return callback(error);
                    } else {
                        if (stat.isFile()) {
                            fs.readFile(jsonPath, function (err, models) {
                                try {
                                    if (err) throw err;
                                    var data;
                                    ...
                                    if (data && data.length) {
                                        ...
                                        if (includeReference) {
                                            if (typeof referenceDepth === 'undefined' || (referenceDepth.currentDepth < referenceDepth.definedDepth)) {
                                                return self.includeReferences(__data, language, undefined, undefined, _inMemory, callback, referenceDepth);
                                            } else {
                            ...
                            return callback(new Error(jsonPath + ' is not of type file!'));
                        }
                    }
                });
...
fileStorage.prototype.find = function (query, options, callback) {
    ...
    try {
        var references = (typeof arguments[3] === 'object' && Object.keys(arguments[3]).length) ? arguments[3] : {},
            parentID = (typeof arguments[4] === 'string') ? arguments[4] : undefined;
        var _inMemory = (typeof arguments[5] === 'object') ? arguments[5] : {};

        if (query && typeof query === 'object' && typeof options == 'object') {
            var self = this;
            var remove = query._remove || false;
            var includeReference = (typeof query.include_references === 'undefined' || query.include_references === true) ? true : false;
            var referenceDepth;

            if (typeof arguments[6] === 'object') {
                arguments[6].currentDepth++;
                referenceDepth = arguments[6];
            } else if (query._referenceDepth || configDefaultRefDepth) {
                referenceDepth = {
                    definedDepth: (query._referenceDepth) ? query._referenceDepth : configDefaultRefDepth,
                    currentDepth: 0
                }
            }
            ...
            if (_inMemory[query.locale] && _inMemory[query.locale][query._content_type_uid]) {
                var data;
                if (query._query) {
                    data = sift(query._query, _inMemory[query.locale][query._content_type_uid]);
                } else {
                    data = _inMemory[query.locale][query._content_type_uid];
                }
                if (data && data.length && (typeof referenceDepth === 'undefined' || (referenceDepth.currentDepth <= referenceDepth.definedDepth))) {
                    ...
                    return self.includeReferences(data, query.locale, references, parentID, _inMemory, callback, referenceDepth);
                } else {
                    return callback(null, data);
                }
            }
            ...
            var model = helper.getContentPath(query.locale);
            ...
            if (jsonPath) {
                        ...
                        if (stat.isFile()) {
                            fs.readFile(jsonPath, 'utf-8', function (err, models) {
                                try {
                                    ...
                                    if (_inMemory[query.locale]) {
                                        _inMemory[query.locale][query._content_type_uid] = models;
                                    } else {
                                        _inMemory = {
                                            [query.locale]: {
                                    ...
                                    if (models && models.length && query._query) {
                                        models = sift(query._query, models);
                                    }
                                    ...
                                    if (includeReference) {
                                        if (typeof referenceDepth === 'undefined' || (referenceDepth.currentDepth <= referenceDepth.definedDepth)) {
                                            ...
                                            return self.includeReferences(__data, query.locale, references, parentID, _inMemory, callback, referenceDepth);
                                        } else {
                                            return callback(null, __data);
                                        }
                                    } else {
            ...
            } else if (query._content_type_uid === assetRouteName) {
                // If sync is not false, get data off inmemory
                if (process.env.SYNC !== 'false') {
                    var results = inmemory.get(query.locale, query._content_type_uid, query._query),
                        data = { assets: (results && results.length) ? results : [] };
                    return callback(null, data);
                } else {
                    // If sync is false, read FS for data
                    var _model = helper.getAssetPath(query.locale),
                        _jsonPath = (query._content_type_uid) ? path.join(_model, query._content_type_uid + '.json') : undefined;
                    if (_jsonPath) {
                        fs.stat(_jsonPath, function (error, stat) {
                            if (error) {
                                return callback(error);
                            } else {
                                if (stat.isFile()) {
                                    try {
                                        fs.readFile(_jsonPath, function (error, result) {
                                            if (error) {
                                                return callback(readErr);
                                            } else {
                                                result = JSON.parse(result);
                                                if (result && result.length && query._query) {
                ...
                helper.generateCTNotFound(query.locale, query._content_type_uid);
            }
        } else {
