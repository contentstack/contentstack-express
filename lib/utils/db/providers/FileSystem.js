...
var defaultReferenceDepth = config.get('storage.options.referenceDepth.defaultRoutes');

console.log(': FileSystem referenceDepth', defaultReferenceDepth);
        ...
        for (var _key in data) {
            ...
            if (typeof data[_key] == "object") {
                if (data[_key] && data[_key]["_content_type_id"]) {
                    if (referenceDepth.currentDepth < referenceDepth.definedDepth) {
                        ...
                        console.error("Skipping reference binding as reference's defined depth has been breached.");
                        data[_key] = [];
                    }
                } else {
...
fileStorage.prototype.findOne = function (query, _callback) {
    try {
        if (query && typeof query === "object") {
            ...
            var referenceDepth = {
                definedDepth: (_query._referenceDepth && typeof _query._referenceDepth === "number") ? _query._referenceDepth: defaultReferenceDepth,
                currentDepth: 0
            };
    ...
    try {
        ...
        if (query && typeof query == "object" && typeof options == "object") {
            ...
            if (_.isPlainObject(arguments[6])) {
                ...
                referenceDepth = {
                    definedDepth: (_query._referenceDepth) ? _query._referenceDepth: defaultReferenceDepth,
                    currentDepth: 0
                }
            }
            ...
            if (_inMemory[language] && _inMemory[language][query._content_type_uid]) {
                ...
                if (data && data.length && referenceDepth.currentDepth <= referenceDepth.definedDepth) {
            ...
            } else if (cachedStructures.indexOf(_query._content_type_uid)) {
    ...
    try {
        if (data && typeof data == "object" && data._content_type_uid && data._uid) {
            ...
            var _callback = function (_data) {
                atomicFS(jsonPath, JSON.stringify(_data), function (error) {
...
fileStorage.prototype.bulkInsert = function (query, callback) {
    try {
        if (query && typeof query == "object" && query._content_type_uid && query.entries) {
            ...
            atomicFS(jsonPath, JSON.stringify(_entries), function (error) {
                if (error) return callback(error);
            ...
            if (!fs.existsSync(jsonPath)) {
                ...
                if (Object.keys(_query).length === 2 && contentTypeId && language) {
                    ...
                    fs.readFile(jsonPath, 'utf-8', function (error, entries) {
                        ...
                        atomicFS(jsonPath, JSON.stringify(entries), function (error) {
                            if (error) return callback(error);
