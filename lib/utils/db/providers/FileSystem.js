...
fileStorage.prototype.processMultipleCT = function (multiReferences, locale, references, parentID, inMemory, callback, referenceDepth) {
  ...
  /**
   * 1. Read multi refs
   * 2. Check if the refs exists inMemory
   * 3. If some content types are pending, make DB calls
   * 4. Hold uids in sequence
   * 5. Do $in in the data for the uids
   * 6. Return result
   */

  // 2.
  /**
   * 1. Format multiReferences and create necessary queries
   * 2. Filter self-refs
   * 3. Make DB calls
   * 4. Get uids
   * 5. Search for matching uids, if match found - add to output and break
   * 6. Return result
   */
  const callbacks = []
  const queries = {}
                      ...
                      if (query._uid && query._uid.$in) {
                        ...
                        if (typeof parseInt(_key) === 'number') {
                          if (__data.length === 1) {
                            ...
                            // better to delete the 'unpublished' object, than return null..
                            // do nothing here..?
                            // delete data[_key];
                            data[_key] = {}
                          } else {
