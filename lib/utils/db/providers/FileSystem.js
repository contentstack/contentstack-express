                ...
                if (data[_key] && data[_key]["_content_type_id"]) {
                    calls.push(function (_key, data) {
                        return (function (_callback) {
                            var query = {"_content_type_uid": data[_key]["_content_type_id"], "_uid": {"$in": data[_key]["values"]},"locale": _locale, "_remove": true},
                                _calls = [];
                ...
                fs.readFile(jsonPath, 'utf-8', function(err, models) {
                    try {
                        ...
                        if (data && data.length) {
                            ...
                            if (includeReference) {
                                self.includeReferences(__data, language, _callback);
                            } else {
            ...
            if (jsonPath && fs.existsSync(jsonPath)) {
                fs.readFile(jsonPath, 'utf-8', function(err, models) {
                    try {
                        ...
                        if (includeReference) {
                            self.includeReferences(__data, language, function(err, results) {
                                if(err) {
                                    callback(err);
                                } else {
