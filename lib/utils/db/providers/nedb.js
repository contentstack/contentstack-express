...
nedbStorage.prototype.includeReferences = function (data, _locale, references, parentID, callback) {
    var self = this,
        includeAssets = (typeof data.__include_assets === 'undefined' || data.__include_assets === true) ? true: false,
        includeEntries = (typeof data.__include_entries === 'undefined' || data.__include_entries === false) ? false: true,
        calls = [];
        ...
        for (var _key in data) {
            ...
            if (typeof data[_key] == "object") {
                if (data[_key] && data[_key]["_content_type_id"]) {
                    if((includeAssets && data[_key]['_content_type_id'] === '_assets') || (includeEntries && data[_key]['_content_type_id'] !== '_assets')) {
    ...
    var recursive = function (data, callback) {
        ...
        if (calls.length) {
            ...
            // Delete internal keys before returning to user
            if(_.has(data, '__include_assets'))
                delete data.__include_assets;
            if(_.has(data, '__include_entries'))
                delete data.__include_entries;
    ...
    try {
        ...
        if (_query && typeof _query == "object") {
            ...
            var remove = _query._remove || false,
                ...
                includeAssets = (typeof _query.include_assets === 'undefined' || _query.include_assets === true) ? true: false,
                options = {};
            ...
            this.db[locale].findOne(_query).sort({"_data.published_at": -1}).exec(function (err, data) {
                try {
                    ...
                    if (data && data._data) {
                        ...
                        if (includeReference || includeAssets) {
                            __data.__include_entries = includeReference;
                            __data.__include_assets = includeAssets;
                ...
                async.parallel(calls, function (err, result) {
                    try {
                        ...
                        if (result && result.entries && result.entries.length) {
                            ...
                            if (includeReference || includeAssets) {
                                _data.__include_entries = includeReference,
                                _data.__include_assets = includeAssets;
                                if (parentID) {
