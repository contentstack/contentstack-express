...
var queryContentstack = require('./request').queryContentstack;
...
helper.unpublishAsset = function(assetUid, lang, assetPath, isRemove, callback) {
  ...
  db.Assets(assetUid).language(lang.code).remove().then(function() {
    return callback();
  }).catch(function(err) {
    return callback(err);
  });
...
helper.deleteAssetFolder = function(assetPath, callback) {
  try {
    ...
    callback(`Error while deleting asset folder\n${error}`);
  }
};
