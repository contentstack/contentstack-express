                        ...
                        switch (schema.data_type) {
                            ...
                            case "text":
                                ...
                                while ((_matches = regex.exec(_entry[parent[j]])) !== null) {
                                    if (_matches && _matches.length && _matches[6] && _matches[4] && _assets[_matches[6]]) {
                                        var tokenValue = _matches[4].split('/')[2],
                                            obj = _assets[_matches[6]];
                                        if (obj['download_id'] && tokenValue && obj['download_id'] == tokenValue) __entry = (schema && schema.field_metadata && schema.field_metadata.markdown) ? __entry.replace(_matches[0], encodeURI(_assets[_matches[6]]._internal_url) + "\n") : __entry.replace(_matches[0], "'" + _assets[_matches[6]]._internal_url + "'");
                                    }
                                }
                                ...
                                while ((_matches2 = regex2.exec(_entry[parent[j]])) !== null) {
                                    if (_matches2 && _matches2.length && _matches2[4] && _matches2[5] && _assets[_matches2[4]]) {
                                        var _obj = _assets[_matches2[4]];
                                        if (_obj['download_id'] && _obj['download_id'] == _matches2[5]) __entry2 = (schema && schema.field_metadata && schema.field_metadata.markdown) ? __entry2.replace(_matches2[0], encodeURI(_assets[_matches2[4]]._internal_url) + "\n") : __entry2.replace(_matches2[0], "'" + _assets[_matches2[4]]._internal_url + "'");
                                    }
                                }
        ...
        try {
            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    var _path_ = [asset];
                    _path_.unshift(_path);
                    var _oldAssetPath = path.join.apply(path, _path_)
                    helper.deleteAssets(assets, asset, _oldAssetPath, function (err, data) {
                        if (!err) {
                            fs.writeFileSync(_assets[lang.code], JSON.stringify(data));
                            async.setImmediate(function () {
                                cb(null, null);
                            });
                        } else {
                ...
                // get RTE assets
                if (assets[asset.download_id] && assets[asset.download_id]['uid'] && assets[asset.download_id]['uid'] === asset.uid) {
                    async.setImmediate(function () {
...
helper.downloadEmbedAssets = function (assetPath, asset, callback) {
    ...
    out.on('response', function (resp) {
        if (resp.statusCode === 200) {
            ...
            if (!fs.existsSync(assetPath)) helper.mkdirAllSync(assetPath, 0755);
            var localStream = fs.createWriteStream(path.join(assetPath, filename));
...
//delete Assets
helper.deleteAssets = function (assets, assetUid, assetPath, callback) {
    if (_.isEmpty(assets)) {
        for (var key in assets) {
            if (key === 'uid' && _.has(assets[key], 'uid') && assets[key]['uid'] === assetUid) delete assets[key];
        }
                    ...
                    getDateFieldValue = function (pattern, value) {
                        ...
                        switch (pattern) {
                            case 'year':
                                ...
                                break;
                            case 'year_short':
                                _value = parseInt(value.getFullYear().toString().slice(-2));
                                break;
                            case 'monthname_short':
                                _value = shortMonthNames[value.getMonth()];
                                break;
                            case 'monthname':
                                _value = monthNames[value.getMonth()];
                                break;
                            case 'month':
                                _value = (value.getMonth() < 9) ? '0' + (value.getMonth() + 1) : (value.getMonth() + 1);
                                break;
                            case 'day':
                                _value = (value.getDate() < 10) ? '0' + value.getDate() : value.getDate();
                                break;
                        }
                        return _value;
                    },
