    ...
    return function (asset, lang, remove, cb) {
        try {
            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (_assetObject) {
                        ...
                        if (fs.existsSync(_assetPath)) {
                            ...
                            if(typeof assetFolderPath != 'undefined' && fs.existsSync(assetFolderPath)) fs.rmdirSync(assetFolderPath);
                        }
                        db
                            .Assets(asset)
                            .language(lang.code)
                ...
                if (rteAssets) {
                    var paths = urlFromObject(rteAssets),
                        _url = getAssetUrl(paths.join('/'));
                    rteAssets._internal_url = _url;
                    async.setImmediate(function () {
                        cb(null, rteAssets);
                    });
                } else {
