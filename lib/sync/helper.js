            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (_assetObject) {
                        ...
                        paths.unshift(_path);
                        var _assetPath = path.join.apply(path, paths);
                        helper.unpublishAsset(asset, lang, _assetPath, function (err, data) {
                            if(!err) {
                                async.setImmediate(function () {
                                    cb(null, null);
                                });
                ...
                var rteAssets = _.find(assets, {'download_id': asset.download_id});
                if (rteAssets) {
                    async.setImmediate(function () {
                        cb(null, rteAssets);
                    });
    ...
    out.on('response', function (resp) {
        if (resp.statusCode === 200) {
            if (asset.download_id) {
                ...
                asset['filename'] =  decodeURI(attachment.split('=')[1]);
            }
            console.log("attachment:::", attachment, 'asset filename: ', asset.filename);
...
// unpublish Assets
helper.unpublishAsset = function(assetUid, lang, assetPath, callback) {
