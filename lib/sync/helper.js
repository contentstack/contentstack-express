    ...
    return request({
        ...
        if (error)
            ...
            if (body.release && body.release.items && body.release.items.length) {
                var _items = body.release.items.length;
...
helper.groupReleaseItems = function (release) {
    var _release = {
        success: [],
        failure: []
    };
    while(release.length > 0) {
        if (release[0].status)
            _release.success.push(release.shift());
        else
            _release.failure.push(release.shift());
    }
    return _release;
}
...
helper.updateRelease = function (release) {
    try {
        var release_uid = release[0]._release_uid,
            _release_name = release[0]._release_name;
        release = this.groupReleaseItems(release);
        release.name = _release_name;
        if (release.failure.length > 0) 
            release.status = 3;
        else
            release.status = 2;
