                    ...
                    if (_entry[parent[j]] instanceof Array) {
                        ...
                        switch (schema.data_type) {
                            ...
                            case "text":
                                ...
                                if (schema && schema.field_metadata && schema.field_metadata.markdown) {
                                    markdownRegEx = new RegExp('(https://(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*))', 'g');
                                    var convertedText = safeConverter.makeHtml(_entry[parent[j]]);
                                    var assets = [];
                                    while ((markdownMatch = markdownRegEx.exec(convertedText)) !== null) {
                                        if (markdownMatch && markdownMatch.length !== 0) {
                                            ...
                                            rteMatch = rteRegEx.exec(markdownMatch[1]);

                                            if (rteMatch && rteMatch !== null && rteMatch !== 0) {
                                                rteMatch[1] = rteMatch[1].replace(/\"$/, '');
                                                // console.log('RTE pattern being replaced in markdown');
                                                assetUrl = rteMatch[1];
                                            } else {
                                                rteMatch[1] = markdownMatch[1].replace(/\"$/, '');
                                            ...
                                            try {
                                                if (matchedAssetObject && matchedAssetObject.url === assetUrl) {
                                                    ...
                                                    console.error('No match found for: ' + matchedAssetObject + '. download_id: ' + download_id + '. assetUrl: ' + assetUrl);
                                                    console.error(JSON.stringify(_assets));
                                                }
                                            } catch (error) {
                                                console.log('@error', error);
                                                console.log('@matchedAssetObject ' + matchedAssetObject, ' | _assets: \n' + JSON.stringify(_assets) + '\n | download_id: ' + download_id);
                                            }
                                        }
                                    }

                                    assets.forEach(function (asset) {
                                        _entry[parent[j]] = _entry[parent[j]].replace(asset.url, asset.replaceUrl);
                                    });
                                } else {
                                    rteRegEx = new RegExp('[\"](https://(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*?))[\"]', 'g');
                                    while ((rteMatch = rteRegEx.exec(_entry[parent[j]])) !== null) {
                                        if (rteMatch && rteMatch.length !== 0) {
                                            ...
                                            if (matchedAssetObject && matchedAssetObject.url === assetUrl) {
                                                ...
                                                console.error('No match found for: ' + matchedAssetObject + ' in');
                                                console.error(JSON.stringify(_assets));
                                            }
                                        }
                                    }
        ...
        var _get = function (schema, _entry) {
            switch (schema.data_type) {
                ...
                case "text":
                    ...
                    if (schema && schema.field_metadata && schema.field_metadata.markdown) {
                        console.log('markdown to html\n' + safeConverter.makeHtml(_entry) + '\n');
                        var convertedText = safeConverter.makeHtml(_entry);
                        markdownRegEx = new RegExp('(https://(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*))', 'g');
                        // var genericRegEx = new RegExp('[(\'\"]https://(?:assets|images).contentstack.io/v3/assets/(?:.*?)/(.*?)/(?:.*?)/(?:.?(?:.*)?)[)\'\"]', 'gm');
                        while ((markdownMatch = markdownRegEx.exec(convertedText)) !== null) {
                            // Apply RTE RegEx match on Markdown field
                            if (markdownMatch && (markdownMatch.length !== 0)) {
                                assetObject = {};
                                console.log('Markdown Match: \n' + JSON.stringify(markdownMatch) + '\n');
                                // console.log('RTE match: ' + rteRegEx.exec(_.cloneDeep(markdownMatch[1])) + '\n');
                                dQuoteRegEx = new RegExp('(https://(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*?))[\"]', 'g');
                                // Check for html image fist
                                dQuoteMatch = dQuoteRegEx.exec(markdownMatch[1]);

                                // Check matches
                                 if (dQuoteMatch !== null) {
                                    console.log('Double Quotes found in Markdown: ' + dQuoteMatch[1]);
                                    // Remove trailing space + double quotes
                                    dQuoteMatch[1] = dQuoteMatch[1].replace(/["]$/, '');
                                    assetUrl = dQuoteMatch[1];
                                    if (dQuoteMatch[5]) {
                                        assetObject.uid = dQuoteMatch[5];
                                    }
                                    if (dQuoteMatch[1]) {
