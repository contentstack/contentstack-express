            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (_asset) {
                        ...
                        delete assets[asset];
                        fs.writeFileSync(_assets[lang.code], 'utf8', JSON.stringify(assets));
                    }
                    async.setImmediate(function () {
                        cb(null, null);
                    });
                ...
                if (assets[asset.download_id]) {
                    var paths = urlFromObject(assets[asset.download_id]),
                        internalUrl = getAssetUrl(paths.join('/'));

                    assets[asset.download_id]['_internal_url'] = internalUrl;
                    async.setImmediate(function () {
