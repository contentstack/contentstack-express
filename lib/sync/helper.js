            ...
            if (assets && assets.length) {
                ...
                if (!_.isEmpty(_assetObject)) {
                    _rteAsset = _.find(assets, function (obj) {
                        // obj.download_id is present only for RT asset
...
function checkEquals (current, published) {
    console.log('474::', current, '--', published)
    if(!_.isEmpty(current) && current.uid === published.uid && current.file_size === published.file_size && current.updated_at === published.updated_at)
        return true;
    return false;
}
...
helper.downloadAssets = function (assetsPath, asset, callback) {
    ...
    out.on('response', function (resp) {
        if (resp.statusCode === 200) {
            ...
            return callback("No file found at given url: " + asset.url, null);
        }
    });
    out.on('error', function (e) {
        return callback("Error in media request: " + e.message, null);
    });
    out.end();
};
