            ...
            if (error)
                ...
                if (body.release && body.release.items && body.release.items.length) {
                    ...
                    _items.forEach(function (item) {
                        ...
                        item._release_name = body.release.name;
                    });
...
helper.groupReleaseItems = function (items) {
    ...
    while(items.length > 0) {
        delete item._release_uid;
        delete item._release_name;
        delete item._isFirst;
        delete item._isLast;
        if (items[0].status === -1){
            _release.failure.push(items.shift());
        } else if (items[0].status === 1) {
            _release.success.push(items.shift());
        } else {
            _release.unprocessed.push(items.shift());
        }
    }
    return _release;
}
...
helper.updateRelease = function (release) {
    try {
        var _release = {
            message: this.groupReleaseItems(release.items),
            name: release.name
        };
        // console.log('@release:', release.uid);
        // _release.name = release.name;
        // _release.uid = release.uid;
        if (_release.message.failure.length > 0)
            _release.status = 3;
        else
            _release.status = 2;
        ...
        return request({
            url: config.get('contentstack.host') + "/" + config.get('contentstack.version') + config.get('contentstack.urls.publish_queue') + _release.uid,
            method: 'PUT',
            headers: headers,
            json: {
                entry: release
            ...
            if (error) {
                ...
                log.error('Release ' + release.uid + '(uid) failed to update status at Contentstack. Error reason::' + JSON.stringify(error));
                console.log('@body', JSON.stringify(body));
            } else {
                console.log('@release update response', JSON.stringify(body));
                log.info('Release ' + _release.uid + '(uid) updated successfully at Contentstack. ');
            }
        });
    } catch (error) {
        log.error('Release ' + release.uid + '(uid) failed to update status at Contentstack. Error reason::' + JSON.stringify(error));
