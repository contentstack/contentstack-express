...
helper.fetchRelease = function(release, callback) {
  ...
  return request({
    ...
    if (body.release && body.release.items && body.release.items.length) {
      ...
      // If release is empty
      return callback(new Error(`Failed to fetch items of ${release.entry.title}: ${release.entry.uid}.\nPlease retry!`));
    }
  });
};
  ...
  if (data && data.schema && data.entry) {
    ...
    var find = function(schema, entry) {
      for (var i = 0, _i = schema.length; i < _i; i++) {
        ...
        case 'reference':
          if (_.isArray(schema[i].reference_to)) {
            hasNewRefFields = true;
            continue;
          }
    ...
    if (hasNewRefFields) {
      sanitizeNewRefFields(data.entry);
    }
  }
  return data;
};
...
/**
 * @summary
 * Maintain _content_type_id. In new reference fields, its _content_type_uid
 * This would break current implementations and add complexity in maintaining 2 different keys i.e. _content_type_id and _content_type_uid
 * @param {*} entry 
 */
function sanitizeNewRefFields(entry) {
  if (entry && typeof entry === 'object') {
    if (entry instanceof Array && entry.length) {
      entry.forEach(function (elem) {
        sanitizeNewRefFields(elem);
      });
    } else if (Object.keys(entry).length) {
      if (entry.uid && entry._content_type_uid) {
        entry._content_type_id = entry._content_type_uid;
        delete entry._content_type_uid;
      } else {
        for (var key in entry) {
          sanitizeNewRefFields(entry[key]);
        }
      }
    }
  }
