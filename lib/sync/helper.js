...
helper.getAssets = function () {
    ...
    return function (asset, lang, remove, cb) {
        try {
            ...
            _assetObject = _.find(assets, {'uid': asset.uid}),
            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (_.isEqual(_assetObject, asset) && !isForceLoad && fs.existsSync(_assetPath)) {
                        ...
                        var rteAsset = _.filter(assets, function(obj) {
                            if(obj.uid === asset.uid && obj.download_id && obj.filename === asset.filename) {
                                return obj;
                            }
                        }),
                            isRemoveOldasset = (rteAsset.length) ? false : true;

                        if (isRemoveOldasset && asset && _assetObject) {
                    ...
                    var __assetObject = _.filter(assets, function (object) {
                        if(object.uid === asset && object._version) {
                            return object;
                        }
                    });
                    if (__assetObject && __assetObject.length && __assetObject[0]) {
                        var  __rteAssets = _.filter(assets, function (object) {
                            if(object.uid === asset && object.download_id && __assetObject[0].filename === object.filename) {
                                return object;
                            }
                        });
                        var paths = urlFromObject(__assetObject[0]);
                        paths.unshift(_path);
                        var _assetPath = path.join.apply(path, paths);
                        var isRemove = (__rteAssets.length) ? false: true;
                        helper.unpublishAsset(asset, lang, _assetPath, isRemove, function (err, data) {
                            if (!err) {
                                async.setImmediate(function () {
                                    cb(null, null);
                                });
