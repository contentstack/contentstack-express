...
var path = require('path'),
    ...
    queryContentstack = require('./request').queryContentstack,
    async = require('async'),
...
helper.fetchRelease = function (release, callback) {
    const release_uid = (release.entry.uid) ? release.entry.uid: release.entry.entry_uid;

    return queryContentstack({
        ...
        if (body.release && body.release.items && body.release.items.length) {
            ...
            // If release is empty
            return callback(new Error(`Failed to fetch items of ${release.entry.title}: ${release.entry.uid}. Please retry!`));
        }
    })
}
...
helper.updateRelease = function (release) {
    ...
    try {
        const failed_items = this.collectFailedItems(release.items);
        const _release = {};
