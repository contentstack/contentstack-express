                            ...
                            case "text":
                                ...
                                while ((_matches = regex.exec(_entry[parent[j]])) !== null) {
                                    if (_matches && _matches.length && _matches[6] && _matches[4] && _assets[_matches[6]]) {
                                        ...
                                        if(obj['download_id'] && tokenValue && tokenValue == obj['download_id']) __entry = (schema && schema.field_metadata && schema.field_metadata.markdown) ? __entry.replace(_matches[0], encodeURI(_assets[_matches[6]]._internal_url) + "\n") : __entry.replace(_matches[0], "'" + _assets[_matches[6]]._internal_url + "'");
                                    }
                                }
                                ...
                                while ((_matches2 = regex2.exec(_entry[parent[j]])) !== null) {
                                    if (_matches2 && _matches2.length && _matches2[4] && _matches2[5] && _assets[_matches2[4]] ) {
                                        ...
                                        if(_obj['download_id'] && _matches2[5] == _obj['download_id']) __entry2 = (schema && schema.field_metadata && schema.field_metadata.markdown) ? __entry2.replace(_matches2[0], encodeURI(_assets[_matches2[4]]._internal_url) + "\n") : __entry2.replace(_matches2[0], "'" + _assets[_matches2[4]]._internal_url + "'");
                                    }
                                }
        ...
        try {
            ...
            if (!asset.download_id) {
                if (!remove) {
                    ...
                    if (_assetObject) {
                        var paths = [_assetObject.uid];
                        paths.unshift(_path);
                        var assetFolderPath = path.join(_path, paths[1]);
                        helper.deleteAssets(assetFolderPath, function (err, data) {
                            if (!err) {
                                db
                                    .Assets()
                                    .language(lang.code)
                                    ...
                                    .spread(function (_assets) {
                                        if (_assets && _assets.length) {
                                            var _entries = _.reject(_assets, {uid: asset});
                                            db.Assets()
                                                .language(lang.code)
                                                .Entry()
                                                .Query()
                ...
                var rteAssets = _.find(assets, {'uid': asset.uid, 'download_id': asset.download_id});
                if (rteAssets) {
                    async.setImmediate(function () {
                        cb(null, rteAssets);
                    });
...
helper.deleteAssets = function (assetPath, callback) {
    try {
