                    ...
                    if (_entry[parent[j]] instanceof Array) {
                        ...
                        switch (schema.data_type) {
                            ...
                            var _matches, regex, __entry;
                                ...
                                // regex for /assets/:api_key/:asset_uid/:download_id/download
                                // var _matches2, __entry2,
                                //     regex2 = new RegExp('https://(pt-|dev-|stag-|)(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/download', 'g');
                                ...
                                // regex for /assets/:api_key/:asset_uid/:download_id/download    
                                var _matches3, __entry3, regex3;
                                if (schema && schema.field_metadata && schema.field_metadata.markdown) {
                                    regex3 = new RegExp('https://(pt-|dev-|stag-|)(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*)', 'g');
                                } else {
                                    regex3 = new RegExp('https://(pt-|dev-|stag-|)(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*?)[\"]', 'g');
                                }
                                __entry3 = _entry[parent[j]].slice(0);
                                while ((_matches3 = regex3.exec(_entry[parent[j]])) !== null) {
                                    if (_matches3 && _matches3.length) {
                                        var _url = _matches3[0].split('"')[0];
                                        var download_id = url.parse(_url).pathname.split('/').slice(4).join('/'),
                                        _obj = _assets[download_id];
                                        if (_obj && _obj['url'] && _obj['url'] == _url) __entry3 = (schema && schema.field_metadata && schema.field_metadata.markdown) ? __entry3.replace(_url, encodeURI(_obj._internal_url) + "\n") : __entry3.replace(_url, _obj._internal_url);
                                    }
                                }
                                _entry[parent[j]] = __entry3;
                                break;
                            }
                        }
                    } else {
        ...
        var _get = function (schema, _entry) {
            switch (schema.data_type) {
                ...
                case "text":
                    ...
                    // for v3 stack
                    // regex for /asset/:api_key/:asset_uid/download_id/download
                    // var _matches2, regex2;
                    // regex2 = new RegExp('https://(pt-|dev-|stag-|)(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/download', 'g');
                    // while ((_matches2 = regex2.exec(_entry)) !== null) {
                    //     console.log("Not statisfied");
                    //     if (_matches2 && _matches2.length) {
                    //         var _assetObject = {};
                    //         _assetObject['is_slash_download'] = true;
                    ...
                    // regex for /asset/:api_key/:asset_uid/download_id/:filename 
                    var _matches3, __entry3, regex3;
                    if (schema && schema.field_metadata && schema.field_metadata.markdown) {
                        regex3 = new RegExp('https://(pt-|dev-|stag-|)(assets|images).contentstack.io/v[\\d]/assets/(.*?)/(.*?)/(.*?)/(.*)', 'g');
                    } else {
                    ...
                    while ((_matches3 = regex3.exec(_entry)) !== null) {
                        if (_matches3 && _matches3.length) {
                            var _assetObject = {},
                            _url = _matches3[0].split('"')[0];
                            if (_matches3[4]) _assetObject['uid'] = _matches3[4];
                            if (_matches3.input) {
