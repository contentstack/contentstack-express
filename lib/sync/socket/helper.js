...
utils.environment = function(url, callback) {
  return request({
    ...
    return callback(null, {
      environment: helper.deleteKeys(response.environment)
    });
  });
};
...
utils.isValid = function(object, env, server) {
  ...
  return (object && object.environment && object.environment.indexOf(env) > -1 && object.action.indexOf('cancelled') === -1 && publishDetails && publishDetails.length && _.findIndex(publishDetails, {
    name: server
  }) != -1);
};