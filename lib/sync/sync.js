 ...
 * 
 * 2. Delete content type from mapper when delete event is called for content type
 *
 * 3. If entry is deleted, remove it from mapper
 *
 * 4. Add mapper to InMemory
 *
 ...
 *
 * Difficult
 * 1. Do not map entries that do not have assets in them
 */
...
Sync.prototype.updateMapper = function (lang, data) {
    return function (callback) {
        try {
             ...
             */
            var mapped_assets = InMemory.get(lang, '_assetMapper', {_data: {_uid: 'assetMapper', _content_type_uid: '_assetMapper'}}, true);
            mapped_assets = (_.isArray(mapped_assets) && mapped_assets.length) ? mapped_assets[0]: {};
            console.log('mapped_assets', JSON.stringify(mapped_assets));
            ...
            if(_.isEmpty(mapped_assets) || typeof mapped_assets === 'undefined') {
                db.ContentType('_assetMapper').Query().toJSON().excludeReference().find().spread(function (mapper_data) {
                    ...
                    _updateMapper(lang, data, mapped_assets, wrapper).then(function () {
...
function _updateMapper (lang, data, mapped_assets, wrapper) {
    return new Promise(function (resolve, reject) {
        ...
        db.ContentType('_assetMapper').language(lang).Entry('assetMapper').update(mapped_assets).then(function (result) {
            ...
            InMemory.set(lang, '_assetMapper', null, {_data: [mapped_assets], _uid: 'assetMapper', _content_type_uid: '_assetMapper'});
            return resolve();
        }, function (err) {
