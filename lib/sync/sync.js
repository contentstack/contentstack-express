...
const statusUpdater = require('./status-manager');
const utils = require('./../utils/index');
const InMemory = require('../utils/db/inmemory');

const config = utils.config;
const context = utils.context;
const db = utils.db;
const log = utils.sync;
const contentTypesUid = '_content_types';
const _routes = '_routes';
const api = config.get('contentstack');
const languages = config.get('languages');
const _types = config.get('contentstack.types');
const _events = config.get('contentstack.events');
...
Sync.prototype.start = function(data) {
  ...
  try {
    ...
    log.error(`Errorred at sync start! ${error.message}`);
    return self.next();
  }
};
    ...
    async.waterfall([
      ...
      function(callback) {
        if (remove) {
          ...
          var d = new Date();
          var _url = `${api.cdn}/${api.version}${api.urls.content_types}${ctUID}${api.urls.entries}${entryUID}`;
          return request({
            url: _url,
            qs: {
      ...
      try {
        if (!error) {
          if (!remove) {
            ...
            db.ContentType(ctUID).language(lang.code).Entry(entryUID).remove().then(function(data) {
              var _updateMapping = self.updateMapper(lang.code, _.cloneDeep(result), 'entries', remove);
              _updateMapping(function(error, success) {
                if (error) {
                  return self.updateEntityStatus({
...
Sync.prototype.fetchAssetDetails = function(uid, _qs) {
  return new Promise(function(resolve, reject) {
    try {
      var _url = `${api.cdn}/${api.version}${api.urls.assets}${uid}`;
      return request({
        url: _url,
        json: true,
        qs: _qs
...
Sync.prototype.updateReferences = function(asset_uid, asset_reference, remove, lang, metadata, eventType, callback) {
  var self = this;
  db.ContentType(asset_reference[0].content_type_uid).Query().toJSON().language(lang).excludeUnpublishDeletion().excludeReference().find().then(function(result) {
        ...
        function(callback) {
          ...
          for (var i in languages) {
            calls.push((function(lang) {
              return function(cb) {
                db.ContentType(ctUID).language(lang.code).Entry().remove().then(function(entries) {
                  db.ContentType(contentTypesUid).language(lang.code).Entry(ctUID).remove().then(function() {
                    db.ContentType(_routes).language(lang.code).Query().toJSON().find().then(function(routeEntries) {
                      ...
                      for (var j = 0, _j = totalRequests; j < _j; j++) {
                        _requests.push(function(j) {
                          return function(_cb) {
                            ...
                            return request({
                              url: `${api.cdn}/${api.version}${api.urls.assets}`,
                              qs: {
                                limit: limit,
                                query: {
                                  uid: {
