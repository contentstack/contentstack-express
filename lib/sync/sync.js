...
/**
 * TODO
 * 1. Remove duplicate paths from asset mapper for single asset { :: Resolved :: }
 * 
 * 2. Delete content type from mapper when delete event is called for content type { :: Resolved :: }
 *
 * 3. If entry is deleted, remove it from mapper { :: Resolved :: }
 *
 * 4. Add mapper to InMemory { :: Resolved :: }
 *
 * 5. Move mapper logic onto a separate file
 *
 * 6. Update documentation
 *
 * Difficult
 * 1. Do not map entries that do not have assets in them { :: Not Possible :: }
 *
 * Bug
 * 1. If single asset is referred multiple times on single field, it will be updated once
 *     - All multiple references of the asset object needs to be updated
 *     { :: Resolved :: }
 *     
 * 2. If multiple groups are present, creates distortion (_.get() doesn't seem to work well if one of the field in path is an array)
 *     - Unable to map references { :: Resolved :: }
 *     - No assets are updated
 */
        ...
        async.waterfall([
            ...
            function (data, callback) {
                if (!remove) {
                    ...
                    async.parallel([
                        self.updateMapper(lang.code, data, 'entries', remove),
                    ...
                    var _updateMapping = self.updateMapper(lang.code, data, 'entries', remove);
                    _updateMapping(function (cb) {
                        console.log('Curried entry delete');
    ...
    return function (callback) {
        try {
            ...
            if(objekt === 'entries' && typeof remove === 'boolean' && !remove) {
                ...
                function get (entry, pathArr) {
                    var i = 0, _pathArrLen = pathArr.length, objs = [];
                    function _get (obj, _path) {
                        if(_.isArray(obj)) {
                            ...
                            if(i === _pathArrLen - 1)
                                ...
                                i++;
                                return _get(obj[_path], pathArr[i]);
                            }
                        }
                    }
                    _get(entry, pathArr[i]);
                    return objs;
                }
                 ...
                 *         {
                 *             uid: _asset_uid_1,
                 *             path: _asset_uid_1_path
                 *         },
                 *         ..
                 *     ]
                 */

                var _mapped_assets = _.compact(_.flattenDeep(asset_paths.map(function (path) {
                    var _tmpAssets = get(data.entry, path.split('.'));
                ...
                // Bug: If an asset is referred multiple times on the same path, multiple paths are created
                // Expected: just one path for such an asset
             ...
             */
            var mapped_assets = InMemory.get(lang, '_assetMapper', {}, true);
            mapped_assets = (_.isArray(mapped_assets) && mapped_assets.length) ? mapped_assets[0]['_data']: {};

            if(_.isEmpty(mapped_assets) || typeof mapped_assets === 'undefined') {
                db.ContentType('_assetMapper').Query().toJSON().excludeReference().find().spread(function (mapper_data) {
                    mapped_assets = (mapper_data.length) ? mapper_data[0]: {};
                    _updateMapper(lang, data, objekt, remove, mapped_assets, wrapper).then(function () {
...
function _updateMapper (lang, data, objekt, remove, mapped_assets, wrapper) {
    return new Promise(function (resolve, reject) {
        if(remove) {
          ...
          *     ]
          */
         // Query builder update itself updates the InMemory, still explictly setting InMemory
        db.ContentType('_assetMapper').language(lang).Entry('assetMapper').update(mapped_assets).then(function (result) {
            ...
            InMemory.set(lang, '_assetMapper', null, [{_data: mapped_assets, _uid: 'assetMapper', _content_type_uid: '_assetMapper'}], true);
            return resolve();
        }, function (err) {
                    ...
                    if (eventType === _events.delete) {
                        helper.deleteAssets(data.asset, lang, function (err, data) {
                            if(!err) {
                                ...
                                console.log(assetUID, asset.object.locale, remove);
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            async.eachLimit(locales, 1, function (lang, cb) {
                ...
                console.log('Starting asset reference updation for:', lang);
                ...
                if(!remove) {
                    ...
                    var _lang_obj = _.find(_locale_objs, {code: lang});
                    ...
                    var paths = urlFromObject(metadata);
                    metadata._internal_url = getAssetUrl(paths.join('/'));
                }

                // Get assets from InMemory, avoid using Query-Builder (uses FS i/o)

                var _mapped_assets = InMemory.get(lang, '_assetMapper', {}, true);
                _mapped_assets = (_.isArray(_mapped_assets) && _mapped_assets.length) ? _.cloneDeep(_mapped_assets[0]['_data']): {};

                if(_.isEmpty(_mapped_assets)) {
                    var _data = _.cloneDeep(_mapped_assets),
                        asset_references = [];

                    // Step 2: Find the 'Content Types' & 'Entry Uids' where it has been referred
                    asset_references = trackAssets(_data, uid, remove);

                    // Step 3: If remove, update the mapper file for the specified language
                    //         Else, find content_type, update each entry where it has been referred, and upsert it backonto the disk
                    return self.realignMapperDocument(_data, asset_references, remove, metadata, lang, cb);
                } else {
                    db.ContentType('_assetMapper').Query().toJSON().language(lang).excludeReference().find().spread(function (result) {
                        var _data = (result.length) ? result[0]: [],
                            asset_references = [];
...
Sync.prototype.realignMapperDocument = function (_data, asset_references, remove, metadata, lang, cb) {
    var self = this;
    if(remove) {
        ...
        if(_.isEmpty(asset_references)) {
            ...
            async.eachLimit(grouped, 1, function (grp, _cb) {
                console.log(grp[0].content_type_uid, 'is under processing!');
...
Sync.prototype.updateReferences = function (asset_reference, metadata, lang, callback) {
    ...
    db.ContentType(asset_reference[0].content_type_uid).Query().toJSON().language(lang).excludeReference().find().spread(function (result) {
        ...
        entries = _.map(entries, function (entry) {
            asset_reference.map(function (obj, index, collection) {
                if(entry.uid === obj.entry_uid) {
                    console.log('Updating content_type_uid', asset_reference[0].content_type_uid, ' entry_uid:', entry.uid);
                    obj.path.map(function (pth) {
                        var assets = _.get(entry, pth);
                        if(_.isPlainObject(assets)) {
                            assign(entry, pth, (assets.uid === metadata.uid) ? metadata: assets);
        ...
        db.ContentType(asset_reference[0].content_type_uid).language(lang).Entry().Query().query({_bulk_insert: true, 'entries': entries}).update().then(function () {
            return callback();
        }, function (err) {
            console.error(err);
