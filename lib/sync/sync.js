 ...
 *     
 * 3. If `contentstack sync` is done, it fails the app { :: Resolved :: }
 * 
 * 4. Bug 2 not yet solved { :: Resolved :: }
 * 
 */
...
Sync.prototype.getAssets = function (entry, pathArr) {
    ...
    function _get (obj, _path, index) {
        
        if(_.isArray(obj)) {
            obj.map(function (sub) {
                _get(sub, _path, index);
            });
        } else if(_.isPlainObject(obj, _path) && _.has(obj, _path)) {
            if(index === _pathArrLen - 1) {
                objs.push(obj[_path]);
            } else {
                index ++;
                return _get(obj[_path], pathArr[index],index);
                
            }
        }
    }
    // Starting with initial index zero
                    ...
                    if (eventType === _events.delete) {
                        helper.deleteAssets(data.asset, lang, function (err, data) {
                            if(!err) {
                                ...
                                self.updateAssetReferences(assetUID, asset.object.locale, remove, {}).then(function () {
                                    return callback(null, null);
                                });
                                // return callback(null, null);
                            } else {
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            async.eachLimit(locales, 1, function (lang, cb) {
                ...
                return self.realignMapperDocument(uid, _data, asset_references, remove, metadata, lang, cb);
            }, function (err) {
                if(err)
                    return reject(err);
                return resolve();
...
Sync.prototype.realignMapperDocument = function (asset_uid, _data, asset_references, remove, metadata, lang, cb) {
    var self = this;
    ...
    async.eachLimit(grouped, 1, function (grp, _cb) {
        ...
        self.updateReferences(asset_uid, grp, remove, lang, metadata, _cb);
    }, function (err) {
        if(err)
            return cb(err);
...
Sync.prototype.updateReferences = function (asset_uid, asset_reference, remove, lang, metadata, callback) {
    var self = this;
            ...
            asset_reference.map(function (obj, index, collection) {
                if(entry.uid === obj.entry_uid) {
                    ...
                    obj.path.map(function (pth) {
                        ...
                        _modify_referred_assets(assets, remove, asset_uid, metadata);
                    });
                }
            });
            return entry;
...
function _modify_referred_assets (assets, remove, asset_uid, metadata) {
