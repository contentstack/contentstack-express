	...
	try {
		...
		var next = function (entityStatus) {
			...
			if (typeof entityStatus === 'object' && entityStatus.hasOwnProperty('_release_uid')) {
				...
				if (entityStatus.hasOwnProperty('_isLast') && entityStatus._isLast) {
					...
					rq_inProgress = false;
					rq_lock = false;
					releaseManager(_release, false, function (error) {
						if (error) {
							console.log('Called release-manager when finished with last element of a release queue.');
							console.error(error);
						}
					});
				}
		...
		var releaseManager = function (release, action, callback) {
			try {
				...
				if (action && !rq_lock && !rq_inProgress) {
					rq_lock = true;
					...
					helper.fetchRelease(release, function (error, items) {
						...
						console.log('@added another release ' + release.entry.title + '(uid: ' + release.entry.uid + ') to release_q');
						...
						items.forEach(function (item) {
							console.log('@added item (uid:' + item.uid + ', content_type_uid:(' + item.content_type_uid + ') to publish-queue');
							...
							if (!_.isEmpty(_locale)) {
								q.push({message: item, lang: _locale});
							} else {
								item.status = 0;
						...
						if (!q_inProgress) {
							if (q.length > 0) {
								...
								if (published_object.hasOwnProperty('_release_uid')) {
									rq_inProgress = true;
									console.log('@release-queue element lock set. Starting sync');
								}
								sync.start(published_object);
								q_inProgress = true;
							}
					...
					for(var i = 0, _i = release_q.length; i < _i; i++) {
						if (release_q[i].entry.uid === release[0]._release_uid) {
							console.log('RQ length', release_q.length);
							console.log('Q length', q.length);
							var _removed_release = release_q.splice(i, 1)[0];
							_release.name = _removed_release.entry.title;
							_release.uid = _removed_release.entry.uid;
					...
					if (release_q.length > 0) {
						releaseManager(release_q.shift(), true, function (error) {
							if (error)
								console.error('Activated next item on release queue. But ran into error.');
							console.log('Next item on release-queue sent for processing');
						});
					}
