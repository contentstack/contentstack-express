		...
		var releaseManager = function (release, action) {
			try {
				...
				if (action && !rq_lock) {
	              ...
	              items[0]._isFirst = true;
	              items[items.length - 1]._isLast = true;
								...
								if (items.length === releases[items[0]._release_uid]) {
									var _release = _.cloneDeep(releases[items[0]._release_uid]);
									delete releases[items[0]._release_uid];
									releaseManager(_release, false);
								}
