		...
		var releaseManager = function (release, action, callback) {
			try {
				...
				if (action && !rq_lock) {
					...
					// TODO: Eval this
					// Does this get added to event loop?
					// 	if so, which one gets priority
					// 		a: Current invocation
					// 		b: Element that's been just published
					// 	Race condition?
					rq_lock = false;
			...
			return function (message, locale) {
				if (message.type === 'release') {
					...
					releaseManager(message, true, function (error) {
						if (error) {
							console.error('Unable to process release-queue.')
							console.error(error);
						}
					});
				} else {
